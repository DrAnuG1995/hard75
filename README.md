...
Photos need IndexedDB, which some browsers restrict on `file://`. If the photo
step misbehaves when opened from disk, use the local server or a hosted copy.
...
- The app nags you if it's been 7 days since the last export, and the calendar
  file adds a weekly backup reminder.
...
5. Two workouts of 45+ minutes, **at least one outdoors**
6. 10 pages read — logged as a book title plus start and finish page, so the
   ...
   and starts at the page you stopped on.
...
count at day 1 tomorrow. Failed attempts are kept in Stats. There's a 12-second
undo on the reset in case of a misclick.

Only **today and yesterday** can be edited. A streak you can back-fill a week
later isn't a streak.
